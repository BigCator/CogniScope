...
class AB3DMOT(object):			  	
	def __init__(self, cfg, cat, calib=None, oxts=None, img_dir=None, vis_dir=None, hw=None, log=None, ID_init=0):                    
		...
		# self.print_param()
		# print(f"ego_com is {self.ego_com}")
	...
	def output(self):
		...
		for trk in reversed(self.trackers):
			...
			if ((trk.time_since_update < self.max_age) and (trk.hits >= self.min_hits or self.frame_count <= self.min_hits)):  
				 # 获取速度
				vx, vy, vz = trk.get_velocity()

				# 拼接输出: [h, w, l, x, y, z, θ, ID, vx, vy, vz, info...]
				# print("trk.info:", trk.info, type(trk.info))
				# print("d shape:", np.array(d).shape, "trk.info shape:", trk.info.shape)


				# results = np.hstack([d, [trk.id, vx, vy, vz], trk.info]).reshape(1, -1)
				row = np.hstack([ np.array(d, dtype=float).ravel(), [int(trk.id), float(vx), float(vy), float(vz)], np.array(trk.info, dtype=float).ravel() ]).reshape(1, -1) 
				results.append(row)
