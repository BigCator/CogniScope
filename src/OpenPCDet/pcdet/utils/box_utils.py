...
def boxes3d_kitti_camera_to_imageboxes(boxes3d, calib, image_shape=None):
    # 如果输入是 Torch Tensor，先转成 NumPy
    if isinstance(boxes3d, torch.Tensor):
        boxes3d = boxes3d.cpu().numpy()

    corners3d = boxes3d_to_corners3d_kitti_camera(boxes3d)  # (N, 8, 3)
    pts_img, _ = calib.rect_to_img(corners3d.reshape(-1, 3))
    corners_in_image = pts_img.reshape(-1, 8, 2)

    min_uv = np.min(corners_in_image, axis=1)  # (N, 2)
    max_uv = np.max(corners_in_image, axis=1)  # (N, 2)
    boxes2d_image = np.concatenate([min_uv, max_uv], axis=1).astype(np.float32)
    image_shape = image_shape.cpu().numpy()

    if image_shape is not None:
        boxes2d_image[:, 0] = np.clip(boxes2d_image[:, 0], 0, image_shape[1] - 1)
        boxes2d_image[:, 1] = np.clip(boxes2d_image[:, 1], 0, image_shape[0] - 1)
        boxes2d_image[:, 2] = np.clip(boxes2d_image[:, 2], 0, image_shape[1] - 1)
        boxes2d_image[:, 3] = np.clip(boxes2d_image[:, 3], 0, image_shape[0] - 1)
