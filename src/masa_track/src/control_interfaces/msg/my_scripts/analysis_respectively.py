...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Road threshold considered for calculation into indicators.")
    parser.add_argument(
        "--category",
        type=str,
        default='modeled',
    ...
    # 获取数据集中有哪些类别
    list_path = 'dataset/kitti/label_list.txt'
    classes_list = []
    ...
    # # 选择要对哪些标签计算指标
    # chengshilukou = ['ChengshiLukou']
    # yewailukou = ['YewaiLukou']
    # modeled = ['Youtong', 'Shu', 'Zhansunzhuangbei', 'Dianxiangan', 'Juma', 'Che', 'Ren', 'Diaobao', 'Zujuehao', 'Danyaoxiang', 'Zhangpeng']
    # few = ['Henggan', 'Tongxinta', 'Tiesiwang']
    # unmodeled = ['Weiqiang', 'Gongmen', 'Langan', 'Ludeng', 'Fangzi', 'Sanjiaozhui', ]
    # if category == 'yewailukou':
    #     calculate_labels = yewailukou
    # elif category == 'chengshilukou':
    #     calculate_labels = chengshilukou
    # exclude_labels =  [item for item in chengshilukou + yewailukou + few + modeled if item not in calculate_labels]
    all_categories = ['ChengshiLukou', 'YewaiLukou', 'Youtong', 'Shu', 'Zhansunzhuangbei', 'Dianxiangan', 'Juma', 'Che', 'Ren', 'Diaobao', 'Zujuehao', 'Danyaoxiang', 'Zhangpeng', 'Henggan', 'Tongxinta', 'Tiesiwang', 'Weiqiang', 'Gongmen', 'Langan', 'Ludeng', 'Fangzi', 'Sanjiaozhui']
    ...
    for i, img_labels in enumerate(results['label']):
        ...
        for j, score in enumerate(img_scores):
            if int(results['label'][i][j]) in [classes_list.index('YewaiLukou'), classes_list.index('ChengshiLukou')]:
                threshold = args.road_threshold
            else:
