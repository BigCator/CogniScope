...
# 使用一个名为 "masa" 的模型对输入图像（或视频帧）进行推理
def inference_masa(
    model: nn.Module,
    img: np.ndarray,
    frame_id: int,
    ...
    """
    # 创建一个字典 data，包含图像数据、帧ID、原始图像形状、图像ID和原始视频长度。
    data = dict(
    ...
    # 如果提供了 text_prompt，则根据 detector_type 将文本提示和自定义实体信息添加到 data 字典中
    if text_prompt is not None:
        if detector_type == "mmdet":
            ...
            data["custom_entities"] = [custom_entities]
    # 如果提供了 test_pipeline，则使用该管道处理 data。
    data = test_pipeline(data)
#     模型推理：

# 使用 torch.no_grad() 确保不计算梯度，减少内存使用并加速推理。
# 将 data 包装成可用来推理的格式。
# 如果提供了 det_bboxes，则将检测到的边界框和标签添加到数据样本中。
    # forward the model
    with torch.no_grad():
