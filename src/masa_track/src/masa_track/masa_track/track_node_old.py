...
class TrackingNode(Node):
    ...
    def obj_callback(self,msg):
        ...
        for obj in msg.data:
            ...
            if obj.id in [0, 1]:
                ...
                # 打印出车辆的速度
                self.get_logger().info('Object ID: %d, Speed: %f' % (obj.id, speed))
    ...
    def listener_callback(self, data):
        ...
        # 处理接收到的图像
        # track_result, fps = inference_masa(self.masa_model, current_frame, 
        ...
        det_time = time.time()
        print(f"检测时间：{det_time - start_time:.3f}s")
        ...
        print(f"跟踪时间：{track_time_end - track_time:.3f}s")
        ...
        end_time = time.time()
        print(f"总计处理第{self.frame_idx}帧的时间：{end_time - start_time:.3f}s")
    ...
    def publish_result_image(self, track_result, frame):
        ...
        for instance in track_result[0].pred_track_instances:
            ...
            for i in range(len(bboxes)):
                ...
                if (label == 2 or label == 8 or label == 7) and score > 0.3:
                    color = [int(c) for c in COLORS[int(track_id) % len(COLORS)]]
                    # 在图像上绘制矩形框
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)),
                                (color[0], color[1], color[2]), 3)
