...
class PointnetSAModuleMSG_Attention(_PointnetSAModuleBase):
    """Pointnet set abstraction layer with specific downsampling and multiscale grouping """
    ...
    def __init__(self, *,
                 ...
                 confidence_mlp: List[int],
                 num_class,
                 use_ema=False,  # 新增EMA开关
                 ema_factor=8):  # EMA分组因子
        super().__init__()
        self.sample_type_list = sample_type_list
        self.sample_range_list = sample_range_list
        self.dilated_group = dilated_group
        ...
        self.pool_weights = nn.ModuleList()
        self.mlps = nn.ModuleList()
        self.ema_modules = nn.ModuleList()  # 新增EMA模块列表
        self.reverse = reverse
        out_channels = 0
        self.sample_idx = sample_idx
        self.saving_cnt = 0
        ...
        for i in range(len(radii)):
            ...
            # ============================================
            if self.use_ema:
                # 获取当前MLP的输出通道数作为EMA的输入通道
                ema_channel = mlp_spec[-1]
                self.ema_modules.append(EMA(ema_channel, factor=ema_factor))
            else:
                # 不使用EMA时添加恒等模块
                self.ema_modules.append(nn.Identity())
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, cls_features: torch.Tensor = None, \
        ...
        if len(self.groupers) > 0:
            for i in range(len(self.groupers)):
                ...
                # ======================= EMA模块（关键修改点） =======================
                new_features = self.ema_modules[i](new_features)  # 应用EMA模块
