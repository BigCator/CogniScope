...
class ChannelAttentionModule(nn.Module):
    def __init__(self, c1, reduction=16):
        ...
        mid_channel = max(1, c1 // reduction)  # 确保至少为1
    ...
    def forward(self, x):
        ...
        channel_att = self.act(avgout + maxout).view(b, c, 1, 1)
...
class SpatialAttentionModule(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv2d = nn.Conv2d(2, 1, kernel_size, padding=kernel_size // 2)
        self.act = nn.Sigmoid()

    def forward(self, x):
        avgout = torch.mean(x, dim=1, keepdim=True)
        maxout, _ = torch.max(x, dim=1, keepdim=True)
        spatial_att = self.act(self.conv2d(torch.cat([avgout, maxout], dim=1)))
...
class CBAM(nn.Module):
    def __init__(self, c1, reduction=16, kernel_size=7):
        ...
        self.channel_att = ChannelAttentionModule(c1, reduction)
        self.spatial_att = SpatialAttentionModule(kernel_size)
...
class PointnetSAModuleMSG_CBAM(_PointnetSAModuleBase):
    """Pointnet set abstraction layer with specific downsampling and multiscale grouping """
    ...
    def __init__(self, *,
                 ...
                 confidence_mlp: List[int],
                 num_class,
                 use_cbam=False,  # 新增CBAM开关
                 cbam_reduction=16):  # CBAM缩减比例
        """
        :param npoint_list: list of int, number of samples for every sampling type
        :param sample_range_list: list of list of int, sample index range [left, right] for every sampling type
        :param sample_type_list: list of str, list of used sampling type, d-fps or f-fps
        ...
        self.sample_range_list = sample_range_list
        self.dilated_group = dilated_group
        self.use_cbam = use_cbam  # 是否使用CBAM
        ...
        self.pool_weights = nn.ModuleList()
        self.mlps = nn.ModuleList()
        self.cbam_modules = nn.ModuleList()  # 新增CBAM模块列表
        self.reverse = reverse
        out_channels = 0
        self.sample_idx = sample_idx
        # ===========================================
        ...
        for i in range(len(radii)):
            ...
            # ============================================
            if self.use_cbam:
                # 获取当前MLP的输出通道数作为CBAM的输入通道
                cbam_channel = mlp_spec[-1]
                self.cbam_modules.append(CBAM(cbam_channel, reduction=cbam_reduction))
            else:
                # 不使用CBAM时添加恒等模块
                self.cbam_modules.append(nn.Identity())
            ...
            for i in range(len(self.groupers)):
                ...
                if (self.training is False) & (save_features_dir is not None):
                    new_features = self.groupers[i](xyz, new_xyz, features,
                                                    save_abs_coord=True)  # (B, C+3, npoint, nsample) point coordinate included
                    save_coords = new_features[:, -3:, :, :]
                    new_features = new_features[:, :-3, :, :]
                # save coordinate from here
                else:
                    new_features = self.groupers[i](xyz, new_xyz,
                                                    features)  # (B, C, npoint, nsample) point coordinate included
                # save_coords = new_features[:,:3, :, :]
                # save pre-pooling features from here
                # =================================================
                # add a learnable weights here before pooling for visualization
                ...
                # 新增CBAM模块应用（关键修改点）
                # ============================================
                new_features = self.cbam_modules[i](new_features)  # 应用CBAM模块
