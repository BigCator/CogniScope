...
class Coord_Backbone(nn.Module):
    """ Backbone for IA-SSD"""
    ...
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        # 获取CoordAtt配置
        coordatt_config = self.model_cfg.get('COORDATT_CONFIG', {})
        use_coordatt_list = coordatt_config.get('ENABLED_LAYERS', [False] * len(sa_config.NSAMPLE_LIST))
        ...
        for k in range(sa_config.NSAMPLE_LIST.__len__()):
            ...
            if self.layer_types[k] == 'SA_Layer':
                ...
                self.SA_modules.append(
                    pointnet2_modules.PointnetSAModuleMSG_Attention(
                        ...
                        use_coordatt=use_coordatt_list[k],  # 传入启用标志
                        coordatt_config=coordatt_config  # 传递完整配置
                    )
                )
