...
class CFAR(Detector3DTemplate):
    # CFAR初始化接收 model_cfg（模型配置）、num_class（类别数量）、
    # dataset（数据集）以及可选的 tb_log（TensorBoard 日志记录器）参数
    # 明确定义了模型的拓扑结构（module_topology）和附加模块（attach_module_topology 和 shared_module_topology）
    # 并通过调用相关构建方法逐步构建这些组件
    def __init__(self, model_cfg, num_class, dataset, tb_log=None):
        ...
        self.attach_model = None if model_cfg.get('DISABLE_ATTACH') else self.build_attach_network()[
            0]  # idx becos it reutrns bb in list
    ...
    # 根据模块拓扑（module_topology）构建网络中的各个模块，并将其依次注册到模型中。
    ...
    # 构建一个特征增强（Feature Augmentation）模块，并将其整合到网络构建流程中
    ...
    def forward(self, batch_dict):
        # batch_dict 是一个字典，包含了模型前向传播所需的输入数据：
        # points: 输入的点云数据，通常是包含 3D 点坐标的张量。
        # frame_id: 当前帧的标识，可能用于多帧跟踪。
        # attach: 附加网络相关的信息，可能用于多模态融合。
        # gt_boxes: 真实框（Ground Truth Boxes），用于训练时的目标检测损失计算。
        # use_lead_xyz: 用于指示是否使用点云的领先坐标。
        # image_shape: 输入图像的尺寸，可能用于将激光雷达和相机图像数据结合。
        # batch_size: 当前批次的大小。
        """
...
class FeatureAug(nn.Module):
    ...
    #
    #     # matching loss
    #     # 基于密度的自适应半径：先用探测半径统计局部密度，再按密度放缩匹配半径；低密度放大，高密度收缩
    #     probe_radius = 1.0
    #     target_density = 4.0  # 期望在 probe_radius 内的平均邻居数
    #     _, density_mask = df.ball_point(16, radar_xyz, radar_xyz, probe_radius)
    #     avg_density = density_mask.float().mean()
    #     base_radius = 1.0
    #     scale = torch.sqrt(target_density / (avg_density + 1e-3))
    #     scale = torch.clamp(scale, 0.7, 1.6)
    #     radius = (base_radius * scale).item()
    #
    #     self_idx, _ = df.ball_point(1, radar_xyz, radar_xyz, radius)
    #     cross_idx, mask = df.ball_point(1, lidar_xyz, radar_xyz, radius)
    #
    #     if mask.sum() == 0:  # fallback 保证至少有一个匹配
    #         _, cross_idx = df.knn_point(1, lidar_xyz, radar_xyz)
    #         mask = torch.ones_like(cross_idx, device=radar_xyz.device, dtype=radar_xyz.dtype)
    #
