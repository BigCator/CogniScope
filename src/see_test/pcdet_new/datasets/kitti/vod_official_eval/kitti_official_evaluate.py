...
def clean_data_modify(gt_anno, dt_anno, current_class, difficulty, roi_clean=False, roi_mode=0):  # per frame
    valid_class_names = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck']
    min_instance_height = [40]
    max_instance_occlusion = [4]

    # default ROI (Driving Corridor in paper)
    left = -8
    right = +8
    max_distance = 25
    # ROI_WIDE
    if roi_clean and roi_mode == 1:
        left = -25.6
        right = +25.6
        max_distance = 40
...
def _prepare_data(gt_annotations, dt_annotations, current_class, difficulty, custom_method=0):
    ...
    for i in range(len(gt_annotations)):
        if custom_method == 0:  # default results
            rets = clean_data_modify(gt_annotations[i], dt_annotations[i], current_class, difficulty)
        if custom_method == 3:  # results in the ROI
            rets = clean_data_modify(gt_annotations[i], dt_annotations[i], current_class, difficulty, roi_clean=True, roi_mode=0)
        if custom_method == 4:  # results in the ROI_WIDE
            rets = clean_data_modify(gt_annotations[i], dt_annotations[i], current_class, difficulty, roi_clean=True, roi_mode=1)
...
def get_official_eval_result(gt_annotations, dt_annotations, current_classes, pr_detail_dict=None, custom_method=0, is_radar=False):
    ...
    if custom_method == 0:
        ...
        print("Evaluating kitti by ROI")
    elif custom_method == 4:
        print("Evaluating kitti by ROI_WIDE")

    # Original OpenPCDet code  此处可以简单进行修改
