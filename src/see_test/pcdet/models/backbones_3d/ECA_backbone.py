...
class ECA_Backbone(nn.Module):
    """ Backbone for IA-SSD with ECA integration"""
    ...
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        self.eca_layers = model_cfg.get('ECA_LAYERS', [])
        self.eca_modules = nn.ModuleDict() if self.use_eca else None
        ...
        # 修复：在收集完所有通道数后初始化ECA模块
        if self.use_eca:
            print(f"Initializing ECA modules at layers: {self.eca_layers}")
            for k in self.eca_layers:
                if k < 0:  # 支持负索引
                    k = len(sa_config.NSAMPLE_LIST) + k
                if k < 0 or k >= len(channel_out_list):
                    raise ValueError(f"Invalid ECA layer index {k}")

                # 获取当前层的输入通道数
                if isinstance(self.layer_inputs[k], list):
                    ch_in = channel_out_list[self.layer_inputs[k][-1]]
                else:
                    ch_in = channel_out_list[self.layer_inputs[k]]

                self.eca_modules[f'layer_{k}'] = eca_layer(ch_in)
                print(f"ECA module added at layer {k} with channel {ch_in}")

        self.num_point_features = channel_out_list[-1]
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(self.SA_modules)):
            ...
            # 在特征上应用ECA模块 (新增部分)
            # =====================================================
            if self.use_eca and hasattr(self, 'eca_modules') and f'layer_{i}' in self.eca_modules:
                # 将点云特征(B, C, N)转换为4D格式(B, C, 1, N)以匹配ECA输入
                B, C, N = li_features.shape
                eca_input = li_features.view(B, C, 1, N)

                # 应用ECA模块
                eca_output = self.eca_modules[f'layer_{i}'](eca_input)

                # 恢复原始维度
                li_features = eca_output.view(B, C, N)
                if torch.isnan(li_features).sum() > 0:
                    raise RuntimeError(f'NaN in ECA output at layer {i}')
