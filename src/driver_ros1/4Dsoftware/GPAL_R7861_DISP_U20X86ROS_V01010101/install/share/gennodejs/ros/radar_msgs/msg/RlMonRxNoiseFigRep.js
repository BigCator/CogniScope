...
class RlMonRxNoiseFigRep {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('rxNoiseFigVal')) {
        this.rxNoiseFigVal = initObj.rxNoiseFigVal
      }
      else {
        this.rxNoiseFigVal = new Array(12).fill(0);
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Check that the constant length array field [rxNoiseFigVal] has the right length
    if (obj.rxNoiseFigVal.length !== 12) {
      throw new Error('Unable to serialize array field rxNoiseFigVal - length must be 12')
    }
    // Serialize message field [rxNoiseFigVal]
    bufferOffset = _arraySerializer.uint16(obj.rxNoiseFigVal, buffer, bufferOffset, 12);
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new RlMonRxNoiseFigRep(null);
    ...
    // Deserialize message field [rxNoiseFigVal]
    data.rxNoiseFigVal = _arrayDeserializer.uint16(buffer, bufferOffset, 12)
  ...
  static Resolve(msg) {
    ...
    const resolved = new RlMonRxNoiseFigRep(null);
    ...
    if (msg.rxNoiseFigVal !== undefined) {
      resolved.rxNoiseFigVal = msg.rxNoiseFigVal;
    }
    else {
      resolved.rxNoiseFigVal = new Array(12).fill(0)
    }
...
module.exports = RlMonRxNoiseFigRep;
