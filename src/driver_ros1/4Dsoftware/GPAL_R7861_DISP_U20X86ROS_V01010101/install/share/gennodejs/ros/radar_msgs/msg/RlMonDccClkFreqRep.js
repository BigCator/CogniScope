...
class RlMonDccClkFreqRep {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.statusFlags = null;
      this.errorCode = null;
      this.freqMeasVal = null;
      this.reserved = null;
      this.timeStamp = null;
    }
    else {
      ...
      if (initObj.hasOwnProperty('freqMeasVal')) {
        this.freqMeasVal = initObj.freqMeasVal
      }
      else {
        this.freqMeasVal = new Array(8).fill(0);
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Check that the constant length array field [freqMeasVal] has the right length
    if (obj.freqMeasVal.length !== 8) {
      throw new Error('Unable to serialize array field freqMeasVal - length must be 8')
    }
    // Serialize message field [freqMeasVal]
    bufferOffset = _arraySerializer.uint16(obj.freqMeasVal, buffer, bufferOffset, 8);
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new RlMonDccClkFreqRep(null);
    ...
    // Deserialize message field [freqMeasVal]
    data.freqMeasVal = _arrayDeserializer.uint16(buffer, bufferOffset, 8)
  ...
  static Resolve(msg) {
    ...
    const resolved = new RlMonDccClkFreqRep(null);
    ...
    if (msg.freqMeasVal !== undefined) {
      resolved.freqMeasVal = msg.freqMeasVal;
    }
    else {
      resolved.freqMeasVal = new Array(8).fill(0)
    }
...
module.exports = RlMonDccClkFreqRep;
