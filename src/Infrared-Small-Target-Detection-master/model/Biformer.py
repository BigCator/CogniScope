...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        #q_win.shape, k_win.shape: tensors torch.Size([3, 49, 256]) torch.Size([3, 49, 256]) 
        #r_weight.shape, r_idx.shape: torch.Size([3, 49, 4]) torch.Size([3, 49, 4])
        # print(q_win.shape, k_win.shape)
        # print()
        # input()
...
class C3BRA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(BiLevelRoutingAttention(c_) for _ in range(n)))
...
if __name__ == "__main__":    
    a = torch.ones(3,256,40,40)
    b = BiLevelRoutingAttention(256)
    c =b(a)
    print(c.shape)