...
class YOLOV5TorchObjectDetector(nn.Module):
    ...
    def forward(self, input_rgb_img, input_ir_img):
        prediction, logits, _ = self.model(input_rgb_img, input_ir_img, augment=False)
        prediction, logits = self.non_max_suppression(prediction, logits, self.confidence, self.iou_thresh,
                                                      classes=None,
                                                      agnostic=self.agnostic)
        self.boxes, self.class_names, self.classes, self.confidences = [[[] for _ in range(input_rgb_img.shape[0])] for _ in
                                                                        range(4)]
