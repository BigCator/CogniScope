...
class Draw3DBox:
    ...
    def publish_3dbox(
        ...
        for i, corners_3d_velo in enumerate(corners_3d_velos):
            ...
            # text_marker.text = f"v={speed:.2f} m/s {state}"
...
class Detection3DNode(Node):
    def __init__(self):
        ...
        self.image_list = []
        self.config_file = '/workspace/ros2_yolo/src/mmdetection3d/configs/mvxnet/mvxnet_fpn_dv_second_secfpn_8xb2-80e_kitti-3d-3class.py'
        # self.checkpoint_file = '/workspace/detection/fusion_det/epoch_80.pth'
        self.checkpoint_file = '/workspace/ros2_yolo/src/fusion_tracker/models/mvxnet_fpn_dv_second_secfpn_8xb2-80e_kitti-3d-3class-8963258a.pth'
        self.ann_file = '/workspace/ros2_yolo/src/fusiondet/config/kitti_infos_val.pkl' 
        self.calib = '/workspace/ros2_yolo/src/fusiondet/config/000001.txt'
        self.save_image_dir = '/workspace/ros2_yolo/src/fusiondet/img_det3d/'
        # 建议调整到更高的值来减少检测数量
        ...
        # Add queues and locks for separating callback and processing
        self.lidar_queue = Queue(maxsize=10)  # Limit queue size to prevent memory issues
        self.image_queue = Queue(maxsize=10)
        self.processing_lock = Lock()
        self.latest_image = None
        self.latest_image_lock = Lock()

        # Start processing thread
        self.processing_thread = Thread(target=self.process_data_loop)
    ...
    def lidar_callback(self, msg):
        """Lightweight callback that just stores data for processing"""
        try:
            # Only put the message in queue if there's space
            if not self.lidar_queue.full():
                self.lidar_queue.put((msg, self.frame_count), block=False)
                self.frame_count += 1
        except:
    ...
    def camera_callback(self, data):
        """Camera callback that stores image for processing"""
        try:
            image = self.cv_bridge.imgmsg_to_cv2(data, desired_encoding="bgr8")
            if not self.image_queue.full():
                self.image_queue.put((image, self.frame_count), block=False)
        except:
            # Queue is full, skip this frame
            pass
        ...
        while rclpy.ok():
            try:
                ...
                if not self.lidar_queue.empty():
                    lidar_msg, frame_id = self.lidar_queue.get(timeout=0.1)
                    
                    # Get the latest available image
                    latest_image = None
                    while not self.image_queue.empty():
                        latest_image, img_frame_id = self.image_queue.get()
                    
                    if latest_image is not None:
                        # Process the data
                        self.process_detection_and_tracking(lidar_msg, latest_image)
                    else:
    ...
    def process_detection_and_tracking(self, lidar_msg, image_np):
        """Process lidar and image data for detection and tracking"""
        start_time = time.time() # ⏱️ 推理开始
        self.frame_id = lidar_msg.header.frame_id

        # 1. 点云转换
        available_fields = [f.name for f in lidar_msg.fields]
        ...
        # 直接从 buffer 解析（最快方式）
        points_struct = np.frombuffer(lidar_msg.data, dtype=point_dtype)
        ...
        if len(tracks) == 0:
            ...
            for trk in tracks:
                ...
                speed = math.sqrt((vx*9+self.ego_vx) ** 2 + (vy*9+self.ego_vy) ** 2 + (vz+self.ego_wz) ** 2)
                self.get_logger().info(f"原始速度分量 - vx: id {int(trk[7])}, {vx:.3f}, vy: {vy:.3f}, vz: {vz:.3f}")
                new_vx = vx + self.ego_vx
                new_vy = vy + self.ego_vy
                new_vz = vz
                self.get_logger().info(f"自车速度分量 - vx: {self.ego_vx:.3f}, vy: {self.ego_vy:.3f}, vz: {self.ego_wz:.3f}")
                self.get_logger().info(f"修正后速度分量 - vx: {new_vx:.3f}, vy: {new_vy:.3f}, vz: {new_vz:.3f}")
                velocities.append(speed)
                state = "MOV" if speed > 5 else "STA"
        ...
        if len(boxes_lidar_filtered) > 0:
            ...
            result_img_msg = self.cv_bridge.cv2_to_imgmsg(img_with_box, encoding="bgr8")
            ...
            self.pub_velo.publish(lidar_msg)
        else:
            msg_bool.data = False
            self.pub_bool.publish(msg_bool)
            result_img_msg = self.cv_bridge.cv2_to_imgmsg(image_np, encoding="bgr8")
            self.pub_image.publish(result_img_msg)
            self.pub_velo.publish(lidar_msg)

        # 9. 设置 MarkerArray 的 frame_id
        if self.no_frame_id:
            self.set_viz_frame_id(lidar_msg.header.frame_id)
